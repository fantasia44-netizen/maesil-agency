...
- `search_creator_content(creator_name, topic_hint="스마트스토어 쿠팡", search_type="video")`
- 결과는 **최신순**으로 반환됨 — **반드시 가장 최근 pub_date 영상**을 선택
- 스마트스토어·쿠팡·온라인셀러·이커머스 관련 영상 중 최신 것 사용
- 검색 결과가 없으면 topic_hint 변경 후 재검색 ("네이버쇼핑 광고", "쿠팡 판매" 등)

**2단계: 이메일 제목 작성 — 친근하고 구체적으로**

좋은 예시 (이렇게 써야 함):
- "[채널명]님 최근 쿠팡 광고비 절감 영상 정말 도움됐어요! 관련해서 제안 드리고 싶어서요 😊"
- "안녕하세요 [채널명]님, 스마트스토어 수익 분석 영상 보고 연락드려요!"
- "[채널명]님 영상 보다가 꼭 소개드리고 싶은 게 생겼어요 :)"

나쁜 예시 (절대 금지):
- "안녕하세요, 제안드립니다" → 너무 딱딱·익명적
- "[채널명]님께 매실인사이트를 소개합니다" → 광고 티 남
- "2025년 최고 영상 감사합니다" → 연도만, 내용 없음
- "귀중한 콘텐츠에 감사드리며…" → 격식체, 거리감

제목 원칙: 친구에게 DM 보내는 느낌 / 짧고 가볍게 / 영상 내용 구체적으로 한 줄만

**3단계: greeting 섹션 작성**
- **1단계에서 선택한 영상 제목을 그대로 인용** — "[영상 제목]" 방식으로
- 그 영상에서 다룬 내용 중 스마트스토어/쿠팡 관련 포인트 1가지 구체적 언급
- "최근에 올리신 / 이번에 올리신" 등 최신성 강조 표현 사용

**절대 금지:**
- 검색 결과에 없는 영상 내용 창작
- 오래된 영상(최신 영상이 있는데 과거 것 선택) 사용
- 연도·날짜만 언급하고 구체적 내용 없는 칭찬
