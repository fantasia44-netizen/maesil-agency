...
export type CmpShareItem = {
  dex: string; name: string; atk: string; tier: string; types: string[];
  fast: CmpMoveDisp; charged: { label: string; color: string; counts: number[] }[];
};
...
export default function CmpShareDense({
  ...
  const build = async () => {
    ...
    try {
      ...
      const COLS = 3, W = 1240, M = 22, gap = 12;
      const colW = Math.floor((W - M * 2 - gap * (COLS - 1)) / COLS);
      const headH = 168, footH = 150, cellH = 208;
      const rows = Math.ceil(items.length / COLS);
      const H = headH + rows * cellH + footH;
      const SCALE = 2;
      ...
      items.forEach((it, i) => {
        ...
        const x = M + col * (colW + gap), y = headH + row * cellH;
        const h = cellH - 12, w = colW;
        ...
        // 빠른기술 칩 (3번째 줄)
        const fy = y + 96; ctx.textAlign = "left"; ctx.font = "700 21px system-ui, sans-serif";
        const fw = ctx.measureText(it.fast.label).width + 20;
        ctx.fillStyle = it.fast.color; ctx.beginPath(); ctx.roundRect(x + 18, fy, fw, 28, 8); ctx.fill();
        ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.fillText(it.fast.label, x + 18 + fw / 2, fy + 20);
        // 차지기술 + 타수
        it.charged.slice(0, 3).forEach((ch, ci) => {
          const cy = y + 134 + ci * 30;
          ctx.fillStyle = ch.color; ctx.beginPath(); ctx.arc(x + 26, cy + 8, 5, 0, Math.PI * 2); ctx.fill();
          const cntTxt = ch.counts.join("·") + hitsUnit;
          ctx.font = "700 21px system-ui, sans-serif";
          const cntW = ctx.measureText(cntTxt).width;
          ctx.textAlign = "left"; ctx.fillStyle = "#334155"; ctx.font = "600 21px system-ui, sans-serif";
          ctx.fillText(trunc(ctx, ch.label, w - 60 - cntW - 16), x + 40, cy + 15);
          ctx.textAlign = "right"; ctx.fillStyle = "#94a3b8"; ctx.font = "700 21px system-ui, sans-serif";
          ctx.fillText(cntTxt, x + w - 16, cy + 15);
        });
      });

      const fy = headH + rows * cellH;
