...
type L = "ko" | "en";
const STAT = { ko: ["공격", "방어", "체력"], en: ["Atk", "Def", "HP"] };

// 스프레드 → 사람이 읽는 IV
const ivStr = (iv: number[]) => iv.join("/");

// 첫 타협(flip 발생) 스프레드 + 잃는 상대
...
export function genArticle(
  ...
  if (ko) {
    ...
    if (fl) verdict.push({ tier: "conditional", iv: `${fl.iv} 부근`,
      note: `${fl.opp ? fl.opp + " 등" : "일부 상대"}를 놓치기 시작합니다. 그 상대를 자주 안 만난다면(실측 픽률 낮음) 감수하고 써도 됩니다.` });
    verdict.push({ tier: "wait", iv: cmpFail ? "공격 14 이하" : "저개체(다수 감소)",
      note: cmpFail ? `${nm}은(는) 같은 종족값 라이벌${rv ? "(" + rv + ")" : ""}·미러에게 동시차징(CMP) 우선권을 무조건 내줍니다. 강화하지 말고 더 좋은 개체를 기다리세요.` : "핵심 매치업을 여러 개 놓칩니다. 급하지 않다면 더 좋은 개체를 기다리는 편이 낫습니다." });
  } else {
  ...
  if (cmpFail || mirrorLost) {
    sections.push(ko ? {
      ...
      body: `마스터리그는 CP 제한이 없어 모두 최대 레벨입니다. 그래서 같은 포켓몬끼리(미러) 또는 종족값이 같은 상대와 같은 턴에 차지무브를 쏘면, 공격 실수치가 높은 쪽이 먼저 터집니다(동시차징·CMP). ${nm}을(를) 공격 14로 키우면 이 우선권 싸움에서 공격 15 상대에게 밀립니다. 실제 시뮬에서 공14 미러는 ${sim.cmp.mirror.map((d) => `실드${d.shields}=${d.result}`).join(", ")}로 나왔습니다. 미러가 잦은 상위 메타에서는 공격 15가 사실상 필수입니다.`,
    } : {
  ...
  if (fl) {
    sections.push(ko ? {
      h: "어디서부터 승패가 갈리나",
      body: onlyHundo
        ? `${nm}은(는) 개체값이 빡빡한 편입니다. ${fl.iv}까지만 내려가도 ${fl.opp ?? "일부 상대"}를 놓치기 시작합니다. 상위 100종을 전수 시뮬한 결과, 스탯이 조금만 낮아져도 브레이크포인트를 넘겨 승패가 바뀌는 매치업이 생깁니다. 그래서 이 포켓몬은 가급적 고개체를 쓰는 게 안전합니다.`
        : `${compromise}까지는 100% 개체와 승패가 같습니다. 그 아래 ${fl.iv}부터 ${fl.opp ?? "일부 상대"}를 놓치기 시작합니다. 딱 그 지점이 브레이크포인트를 넘는 구간입니다.`,
    } : {
  ...
  const faq = ko ? [
    { q: `${nm}, 100% 아니면 못 쓰나요?`, a: onlyHundo
        ? `${nm}은(는) 개체값이 빡빡한 편이라 실질 100%에 가까운 게 안전합니다. ${fl ? fl.iv + "부터 " + (fl.opp ?? "일부 상대") + "를 놓칩니다." : ""} 다만 놓치는 상대의 실측 픽률이 낮다면 실전 손실은 생각보다 작을 수 있습니다.`
  ...
  const lead = ko
    ? `${nm}은(는) ${cmpFail ? "공격 15가 사실상 필수이고, " : ""}${onlyHundo ? "개체값이 빡빡한 편이라 고개체가 안전합니다." : `타협선은 ${compromise} 정도입니다.`} 마스터리그 상위 100종을 배틀 시뮬레이터로 전수 대입하고, 미러전·${rv ? "라이벌 대면·" : ""}베스트파트너까지 계산한 결과를 아래에 정리했습니다. (100% 실드1 성적 ${bs ? bs.wins + "승 " + bs.losses + "패" : ""})`
