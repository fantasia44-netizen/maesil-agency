...
export const PRIVACY: Record<Locale, LegalDoc> = {
  ko: {
    ...
    sections: [
      ...
      { h: "4. 이용자 권리", body: ["브라우저 설정에서 쿠키를 차단·삭제할 수 있습니다. 서비스는 개인정보를 직접 저장하지 않으므로 열람·삭제 요청 대상 데이터가 없습니다. 문의는 아래 연락처로 주세요."] },
  ...
  en: {
    ...
    sections: [
      ...
      { h: "4. Your rights", body: ["You can block or delete cookies in your browser settings. As the Service does not store personal data directly, there is no personal data to access or delete. For questions, use the contact below."] },
  ...
  ja: {
    ...
    sections: [
      ...
      { h: "4. 利用者の権利", body: ["ブラウザ設定でクッキーを拒否・削除できます。本サービスは個人情報を直接保存しないため、開示・削除の対象データはありません。ご質問は下記まで。"] },
  ...
  "zh-TW": {
    ...
    sections: [
      ...
      { h: "4. 您的權利", body: ["可於瀏覽器設定封鎖·刪除 Cookie。本服務不直接儲存個人資料，故無可查閱·刪除之個資。有問題請由下方聯絡。"] },
...
export const ABOUT: Record<Locale, LegalDoc> = {
  ko: {
    ...
    sections: [
      ...
      { h: "연락처", body: ["문의: tcgnote 운영자 (사이트 하단 연락 경로)."] },
    ],
  },
  en: {
    ...
    sections: [
      ...
      { h: "Contact", body: ["Inquiries: TCG Note operator (contact via site footer)."] },
    ],
  },
  ja: {
    ...
    sections: [
      ...
      { h: "連絡先", body: ["お問い合わせ: TCG Note 運営者(サイトフッターの連絡経路)。"] },
    ],
  },
  "zh-TW": {
    ...
    sections: [
      ...
      { h: "聯絡", body: ["洽詢：TCG Note 營運者(透過網站頁尾聯絡)。"] },
    ],
  },
};
